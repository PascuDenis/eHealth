...
import { EditProfileService } from "src/app/core/services/edit-profile.service";
...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.specialistService
      .getSpecialistById(this.authService.getAuth().currentUser.uid)
      .pipe(
        tap((user) => {
  ...
  populateSelect() {
    this.editProfileService.getSpecialisations().subscribe((res) => {
  ...
  addSpecialization(value) {
    if (value == null || value == "") {
      ...
      this.editProfileService.saveSpecialization(value);
    }
  }
  ...
  removeSpecialization(value) {
    const dialogRef = this.dialog.open(GeneralDialogComponent, {
      width: "350px",
      data: "Are you sure you want to delete this specializtion?",
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        console.log("Yes clicked");
        this.editProfileService.deleteSpecialization(value);
      }
    });
  }
  ...
  uploadProfilePicture(event) {
    ...
    console.log(event);
    this.specialistService.uploadProfileImage(
      this.authService.getAuth().currentUser.uid,
      this.profileImage
    );
  }
